...
def run_training(filename, n_trajs, n_init_states):
    ...
    state_space_bounds = np.array([[40,40], [40,40]]) #, [10,10], [10,10]])

    sir_dataset = AbstractionDataset(n_init_states, n_trajs, state_space_bounds, 'Voter', time_step, T)
...
def plot_trajs(filename):
    ...
    if len(np.shape(objects['X']))==3:   # training: all trajs without splitting of initial states
        plt.plot(objects['X'][:,:,0].T, label='X', c='C0')
        plt.plot(objects['X'][:,:,1].T, label='Y', c='C1')
        #plt.plot(objects['X'][:,:,2].T, label='Zx', c='C2')
...
def main():
    ...
    run_training("/Users/juliaklein/Documents/robots/Voter_training_set_test_Z1.pickle", n_trajs = 1, n_init_states = 1)
    ...
    plot_trajs("/Users/juliaklein/Documents/robots/Voter_training_set_test_Z1.pickle")
